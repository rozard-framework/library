...
/***  TABSE MODULE */
    ...
    class tabs{
        ...
        constructor( data ) {
            ...
            this.actions = Array.prototype.slice.call( this.parent.querySelectorAll( '.tabs-action' )); 
            this.content = Array.prototype.slice.call( this.parent.querySelectorAll( '.tabs-content' )); 
            this.open();
        }
...
/***  APP THEMES */
    ...
    let appthemes = ()=> {
        ...
        let prep = new core;
        let data = prep.get_ids('app_theme');
        let sets = prep.get_all('.set-theme');
        let save = prep.get_ids('submit');

        sets.forEach( set => {
            set.addEventListener('click', (event) => {
                let check = event.currentTarget.querySelector('.toggle-checkbox').checked;
                if (check === true) {
                    data.value = event.currentTarget.dataset.theme;
                    save.click();
                }
                else {
                    data.value = '';
