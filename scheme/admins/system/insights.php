...
if ( ! class_exists( 'rozard_system_insight' ) ) {


    class rozard_system_insight{
        ...
        private function data_view() {
            ...
            // module layout
            if ( isset( $system['structure']['layout']['insight'] ) ) {

                $this->raws['layout'] = $system['structure']['layout']['insight'];
            }
            else {
        ...
        private function data_core() {
            ...
            $dashboard = array(
                ...
                'sort' => 10,
                'form' => '',
                'link' => network_admin_url('index.php?node=general') ,
            );
            ...
            $this->raws['node']['general']   = $dashnav;
            $this->raws['data']['general'][] = $dashboard;
        ...
        private function data_exts() {
            ...
            //  validate extend
            if ( empty( $system['insight'] ) ) {
                return;
            }


            // extend menu
            foreach( $system['insight'] as $key => $item ) {

                // crafting
                $slug = str_slug( $item['keys'] );
                $node = str_slug( $item['node'] );
                $item['link'] = network_admin_url( 'index.php?page=panel-' . $slug .'&node='. $node )  ;
               

                // register 
                $this->insight['data'][$node][] = $item;
